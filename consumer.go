...
func (c *Consumer) OriginSource(source string, parsedLine, parsedColumn int) []OriginLine {
	if len(c.sections) == 0 {
		return nil
	}

	Sources := c.sections[0].Map.Sources
	SourcesContent := c.sections[0].Map.SourcesContent

	sourcesContentIndex := -1
	for i, s := range Sources {
		if source == s {
			sourcesContentIndex = i
			break
		}
	}

	sourceCode := SourcesContent[sourcesContentIndex]
	...
	for i := start; i < end; i++ {
		code = append(code, OriginLine{
			Highlight: i+1 == parsedLine,
			Number:    i + 1,
			Code:      lines[i],
		})
	}

...
func TestOriginSource(t *testing.T) {
	file, err := ioutil.ReadFile("assets/SdkTest.1db18e51.js.map")
	if err != nil {
		panic(err)
		return
	}

	parse, err := sourcemap.Parse("", file)
	if err != nil {
		panic(err)
	}
	source, name, line, column, ok := parse.Source(1, 201)
	fmt.Println(source, name, line, column, ok)

	codes := parse.OriginSource(source, line, column)

	//for _, code := range codes {
	//	fmt.Println(code.Code)
	//}
	marshal, err := json.MarshalIndent(codes, "", " ")
	if err != nil {
		t.Fatal(err)
	}
